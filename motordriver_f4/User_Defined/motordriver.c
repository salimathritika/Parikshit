 ...
 * return type- void
 */
void transfer(TIM_HandleTypeDef *tm2,TIM_HandleTypeDef *tm3,TIM_HandleTypeDef *tm1,TIM_HandleTypeDef *tm4,double *d_dutyx,double *d_dutyy,double *d_dutyz,double *f_dutyx,double *f_dutyy,double *f_dutyz)
{
//	tim2=tm2;
//	tim3=tm3;
//	tim1=tm1;
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)  // if the interrupt source is channel1
	{
		...
		else   // if the first is already captured
		{
			...
			{
				Difference_x = (0xffffffff - IC_Val1x) + IC_Val2x;
			}

			double refClock = TIMCLOCK/(PRESCALAR);
			double mFactor = 1000000/refClock;
			fbx_duty = Difference_x*mFactor;
	...
	else if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2)  // if the interrupt source is channel1
		{
			...
			else   // if the first is already captured
			{
				...
				{
					Difference_y = (0xffffffff - IC_Val1y) + IC_Val2y;
				}

				double refClock = TIMCLOCK/(PRESCALAR);
				double mFactor = 1000000/refClock;
				fby_duty = Difference_y*mFactor;
	...
	else if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_3)  // if the interrupt source is channel1
		{
			...
			else   // if the first is already captured
			{
				...
				{
					Difference_z = (0xffffffff - IC_Val1z) + IC_Val2z;
				}

				double refClock = TIMCLOCK/(PRESCALAR);
				double mFactor = 1000000/refClock;
				fbz_duty = Difference_z*mFactor;
...
void maintain_rpm_x()
{
	...
	while(1)
	{
		...
		while(prevd==duty_dx)
		{
			...
			{
				set_rpm_motorx((fbx_duty-1),frx); //decreasing duty by 1% to get closer to desired rpm
			}
			else if((fbx_duty<(duty_dx-0.5)))
			{
				...
				set_rpm_motorx((fbx_duty-1),frx); //increasing duty by 1% to get closer to desired rpm
			}
		}
	}
}
...
void maintain_rpm_y()
{
	...
	while(1)
	{
		...
		while(prevd==duty_dy)
		{
			...
			{
				set_rpm_motorx((fby_duty-1),fry); //decreasing duty by 1% to get closer to desired rpm
			}
			else if((fby_duty<(duty_dy-0.5)))
			{
				...
				set_rpm_motorx((fby_duty-1),fry); //increasing duty by 1% to get closer to desired rpm
			}
		}
	}
}
...
void maintain_rpm_z()
{
	...
	while(1)
	{
		...
		while(prevd==duty_dz)
		{
			...
			{
				set_rpm_motorx((fbz_duty-1),frz); //decreasing duty by 1% to get closer to desired rpm
			}
			else if((fbz_duty<(duty_dz-0.5)))
			{
				...
				set_rpm_motorx((fbz_duty-1),frz); //increasing duty by 1% to get closer to desired rpm
			}
		}
	}
}
