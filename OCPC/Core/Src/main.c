...
/* USER CODE BEGIN PV */
volatile uint16_t OCPC_Status=0x0000;
volatile uint16_t OCPC_Fault_Status=0x0000;
/* USER CODE END PV */
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	 ...
	 /* void EXTI2_IRQHandler(void)
	  {
		  if (HAL_EXTI_GetPending(EXTI_HandleTypeDef *hexti, EXTI_TRIGGER_RISING_FALLING))
		  {
